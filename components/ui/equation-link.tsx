...
export default function EquationLink({
  ...
  if (type === "grid")
    return (
      <Link
        href={`/equations/${slug}`}
        className="border border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700 hover:shadow-sm rounded-2xl overflow-clip transition-transform active:scale-95"
      >
        <div className="px-6 py-3">
          <LatexPreview latex={latex} hideLatexCode />
        </div>
        <hr />
        <hgroup className="px-6 py-5 *:p-0 bg-zinc-100 dark:bg-zinc-900 flex flex-col gap-1 h-full">
